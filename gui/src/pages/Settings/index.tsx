...
const SettingsComponent = (props: SettingsProps, ref: React.Ref<SettingsHandle>) => {
      ...
      if (config) {
        ...
        if (config.voice) {
          ...
          setVoiceConfig(prev => ({
            ...
            ttsProvider: normalizeTtsProvider(config.voice.ttsProvider),
  ...
  const saveAllConfigs = useCallback(async () => {
    ...
    try {
      ...
      toast.success('配置已保存')
      // 2026-09-10 U盘验收: 保存后立即重读——密钥类字段是否真的落盘当场可见,
      // 不再需要"切走再切回"才能发现丢失(百度 key 二次实测教训)
      try { await loadConfigs(true) } catch (e) { console.warn('[Settings] 保存后重读失败:', e) }
    } catch (e: any) {
