 ...
 * 消费 GET /api/cockpit/overview（Task A1）：五张健康卡（插件/技能/专家/今日用量/MCP）
 * + 告警聚合条 + 数据源降级详情 + 后端进程诊断小字 + 快捷动作。
 * onNavigate 由 ManagementCockpit 传入（切 tab）；预算阈值读 localStorage.costDashboard.*。
 * 单源失败对应 data 段为 null → 卡片显示「—」+「降级」副题。
 */
import { useEffect, useState } from 'react'
import { Puzzle, Sparkles, BrainCircuit, Gauge, Plug, ArrowRight, RefreshCw, AlertTriangle } from 'lucide-react'
...
export default function OverviewPanel({ onNavigate }: { onNavigate: (tab: CockpitTab, section?: string | null) => void }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* 2026-09-09 老板视角: 后端进程信息(Node 版本/内存/uptime)从健康卡降为诊断小字——
          工程师字段不占老板第一屏; backend 源降级时整行不渲染(整页已处于降级态)。 */}
      {data.backend && (
        <div className="text-[11px] theme-text-muted">
          后端进程：Node {data.backend.nodeVersion} · 运行 {data.backend.uptime}s · 内存 {data.backend.memHeapMb}MB
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {[
          ...
          // 2026-09-09 老板视角: 消息通道(飞书/企微)是老板最常配置的分区, 借 navSection
          // 深链提到快捷操作首位(channel section id 见 Settings 导航表)。
          { label: '消息通道', tab: 'settings' as CockpitTab, section: 'channel' },
