...
// 2026-09-09 老板视角分组: 常用 tab 前置(总览/系统设置/专家/技能/用量),
// 连接(MCP)/插件归入「高级」段(分隔线隔开, 可达性不变)——MCP 的设备服务名/
// connectTimeout 等开发者字段与插件装配概念对非技术用户是噪音(2026-09-09 管理舱审查轮)。
const TABS_COMMON: { id: CockpitTab; label: string; icon: any }[] = [
...
const TABS_ADVANCED: { id: CockpitTab; label: string; icon: any }[] = [
      ...
      <div className="voice-shell-config-mask" onClick={requestClose}>
        <div className="voice-shell-config-full" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="cockpit-tabbar">
            ...
            <span className="mx-1 h-4 w-px self-center bg-zinc-500/30" aria-hidden />
            {TABS_ADVANCED.map(t => (
              <button
                key={t.id}
                type="button"
