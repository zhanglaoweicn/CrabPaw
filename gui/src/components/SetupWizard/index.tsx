...
export function SetupWizard({ onComplete, onStartService }: SetupWizardProps) {
  ...
  // 音色列表来自探测到的厂商——初始值(默认 edge 晓晓)与列表不匹配时归到第一项
  useEffect(() => {
    const opts = ttsDetected?.voiceOptions
    if (opts?.length && !opts.some(v => v.value === tasteVoice)) setTasteVoice(opts[0].value)
  }, [ttsDetected])
  ...
  const playTasteVoice = async () => {
    ...
    try {
      ...
      if (!data || !data.audioBase64) {
        // 2026-09-10 U盘验收: 豆包音色与 resourceId 不匹配时引擎会静默降级女声,
        // 必须把真实错误透出(如 "resource ID is mismatched"), 否则用户以为选错了自己
        const errMsg = String(data?.error || res?.error || '')
        // 2026-09-11: 豆包按音色包开通资源——音色不在当前 resource 覆盖范围时
        // 报 "resource ID is mismatched with speaker related resource" 并降级默认女声。
        // 识别该场景给可行动指引, 而不是让用户以为选错了按钮。
        if (/mismatch/i.test(errMsg)) {
          setTasteError('该音色不在当前资源包内：请到火山引擎控制台「音色管理」核对你账号可用的音色，或先试听晓鹤（默认）')
          return
        }
        setTasteError('合成失败' + (errMsg ? '：' + errMsg.slice(0, 120) : '，请重试'))
        return
      }
  ...
  // 2026-09-11 跳过入口: 未购买 key 的用户可先体验产品全貌, 之后在设置中随时配置
  const skipSetup = async () => {
    try { await apiPost('/config', { setupCompleted: true }) } catch (e) { console.warn('[setup] 跳过标记写入失败:', e) }
    onComplete()
  }
  ...
  const testWakeWord = async () => {
    ...
    if (wakeOnHitUnsubRef.current) { wakeOnHitUnsubRef.current(); wakeOnHitUnsubRef.current = null }
    // 2026-09-10 U盘验收修复: 待测词先注册进 KWS——否则引擎听的是旧词表(首启为空/默认词),
    // 用户喊向导里新选的唤醒词永远 0 命中("测试唤醒词失败")。与正式保存同款词表结构。
    try {
      const words = [...new Set([wakeWord, '小龙女'])].filter(Boolean)
      await w.wake.setKeyword(words)
    } catch (e) { console.warn('[setup] 注册待测唤醒词失败:', e) }
    ...
    <div className="flex items-center justify-center min-h-full p-6 theme-bg-primary">
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginTop: 10 }}>
          ...
          >
            暂无 API Key？先跳过，进入后可在 系统设置 → 模型配置 中随时配置
          </button>
        </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              {wakeTestResult !== null && (
                <span style={{
                  ...
                  命中 {wakeTestResult.hits}/3 — {wakeTestResult.hits >= 2 ? '效果好，可继续' : wakeTestResult.hits === 0 ? '未检测到声音：请确认电脑已连接麦克风，并在麦克访问提示中点允许后重试' : '建议换一个词或换个语调再试'}
                </span>
              )}
            </div>
            {wakeSaveError && (
        ...
        {step === 'taste' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, paddingTop: 16, paddingBottom: 16 }}>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
              {/* 2026-09-10 U盘验收修复: 试听音色必须与实际 TTS 服务商匹配——
                  曾硬编码 edge 音色, 豆包不认这些 id 全部回退默认晓鹤("试听都是晓鹤/无效")。
                  列表改用激活步骤探测到的厂商音色表; 未配置 TTS 时给提示而非假按钮。 */}
              {ttsDetected?.voiceOptions?.length ? ttsDetected.voiceOptions.map(v => (
                <button
                  key={v.value}
                  onClick={() => setTasteVoice(v.value)}
                  style={{
                    padding: '8px 14px', borderRadius: 8, border: '1px solid',
                    borderColor: tasteVoice === v.value ? 'var(--accent-primary)' : 'var(--border-primary)',
                    color: tasteVoice === v.value ? 'var(--accent-primary)' : 'var(--text-primary)',
                ...
                <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>
                  当前未配置 TTS 服务商(可返回上一步填写豆包/OpenAI 等 Key 后再试听)
                </span>
              )}
            </div>
