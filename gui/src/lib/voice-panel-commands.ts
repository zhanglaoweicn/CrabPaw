...
export const RULES: CommandRule[] = [
  ...
  // ---- 演示模式（2026-09-22 体验层）——投屏/例会时把界面收成"只留议题与结论" ----
  // 开/关分列两条（不用一条 toggle）：说"退出演示模式"时若本就没开，不该反而打开。
  {
    kind: 'present',
    action: 'open',
    patterns: [
      /^(打开|进入|开始|开启)(演示模式|投屏模式|演讲模式)$/,
      /^演示模式$/,
      /^投屏模式$/,
    ],
  },
  {
    ...
    action: 'close',
    patterns: [
      /^(退出|关闭|结束|关掉)(演示模式|投屏模式|演讲模式)$/,
      /^退出投屏$/,
    ],
  },
  // 2026-09-24 会客厅轮(S3): 会客厅模式（放大球/收起遥测/客人面前隐藏明细）。
  // 措辞刻意避开"投屏模式"——那条已属演示模式；两者语义不同：演示模式是"临时只留
  // 结论"，会客厅模式是"这台机器当前的形态"。
  {
    kind: 'kiosk',
    action: 'open',
    patterns: [
      /^(打开|进入|开启|切换)(会客厅模式|一体机模式|客人模式|展示模式)$/,
      /^会客厅模式$/,
      /^一体机模式$/,
    ],
  },
  {
    ...
    action: 'close',
    patterns: [
      /^(退出|关闭|结束|关掉)(会客厅模式|一体机模式|客人模式|展示模式)$/,
      /^退出会客厅$/,
      /^退出展示模式$/,
    ],
  },
  ...
  // 不再跳转业务面板——2026-08-19 业务面板已退役）。
  {
    ...
    action: 'open',
    patterns: [
      /^(打开|显示|查看)?(今天|今日)?(的)?(日程卡片|日程卡|日历卡片|日历卡|日程|日历|日程安排)$/,
      // 2026-09-21: 晨报带「日程」块发"打开日程"直开面板; "打开今日日程"等
      // 指令动词开头变体同命中(查询语义句式仍走原模式, 不扩大拦截面)
      /^(今天|今日)有什么日程$/,
      /^今天的日程$/,
