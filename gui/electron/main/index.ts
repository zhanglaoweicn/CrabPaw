...
function writeJsonFile(filePath: string, data: any): { success: boolean; error?: string } {
  try {
    ...
    // 2026-09-10 (U盘验收): 双写者竞态根治——设置页密钥经后端 HTTP(POST /config)落盘后,
    // 桌面侧任何整包写回(config:set 唤醒词/自动更新检查)若携带旧快照, 会用不含新密钥的
    // 视图覆盖 CONFIG_PATH, 用户刚存的 key 静默丢失(百度搜索 key 二次实测)。
    // 对密钥字段做"新值空/掩码则保留盘上旧值"的保全合并(与后端 config-handler 掩码保护同款)。
    if (filePath === CONFIG_PATH) {
      const SECRET_SECTION_FIELDS: Array<[string, string[]]> = [
        ['search', ['tavilyApiKey', 'bingApiKey', 'baiduApiKey']],
        ['voice', ['doubaoKey', 'doubaoAccessKey', 'volcanoToken', 'volcAsrApiKey', 'openaiApiKey', 'qwenApiKey', 'aliyunApiKey']],
        ['lark', ['appSecret']],
        ['wecom', ['secret']],
      ]
      const diskNow = readJsonFile(CONFIG_PATH)
      if (diskNow && typeof diskNow === 'object') {
        for (const [section, fields] of SECRET_SECTION_FIELDS) {
          const diskSection = (diskNow as any)[section]
          if (!diskSection || typeof diskSection !== 'object') continue
          data[section] = data[section] || {}
          for (const f of fields) {
            const cur = (data[section] as any)[f]
            const old = (diskSection as any)[f]
            const curMissingOrMasked = !cur || (typeof cur === 'string' && cur.includes('****'))
            if (curMissingOrMasked && typeof old === 'string' && old && !old.includes('****')) {
              ;(data[section] as any)[f] = old
            }
          }
        }
      }
