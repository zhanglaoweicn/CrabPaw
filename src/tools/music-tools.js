    ...
    parameters: {
      ...
      properties: {
        action: {
          ...
          description: '操作类型: search=搜索歌曲(需query, 返回tracks含id可直接用于play), play=播放(三种传法任选: id / url+title / 直接传query歌名自动搜索播放第一首——无需先拿ID), stop=关闭面板, pause=暂停/恢复, next=下一首, prev=上一首, set_volume=调音量, seek=跳转进度'
        },
        query: {
          type: 'string',
