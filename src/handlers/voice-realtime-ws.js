...
// (实测"接着说几点了没反应"= 尾窗丢音频)。
// 2026-09-25: 上限 2s → 8s。实机日志里补播字节数反复贴着旧上限(61440/57344),
// 说明用户在图播报期说了一整句、超出部分被丢 → 模型只收到残段 → 答非所问或没反应。
// 放宽到 8s 可容下一整句; 代价是补播按实时节奏回灌(32000 B/s), 极端情况下用户
// 新语音要排在补播之后 → 权衡后取"不丢话"优先(丢话的代价是整句作废)。
const TAIL_BUFFER_MAX = 256000;
...
function createRtConnection(ws, deps = {}) {
  ...
  ws.on('message', (raw, isBinary) => {
    ...
    switch (msg.type) {
      case 'ping':
        // 2026-09-25: 协议层心跳回包——客户端据此判链路半开(真死连接直接探测, 不再靠
        // "没听见话"间接猜)。与业务无关: 哪怕正在等主链路长答案, 心跳也必须立刻回,
        // 否则客户端会把"业务静默"当成"连接已死"。
        toClient({ type: 'pong', ts: msg.ts });
        break;
      case 'mute':
        clientMuted = true;
        setMute(true);
