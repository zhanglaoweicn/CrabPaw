...
- PlantUML：Kroki API（零本地依赖）→ plantuml CLI → java -jar plantuml.jar → docker
- Graphviz：`dot`
...
# ----- PlantUML via Kroki API (zero local deps) -----


def _render_plantuml_via_kroki(source: str, out: Path, pdf_fit: bool = True,
                                scale: float = 3.0) -> str:
    """Render PlantUML via Kroki public API. Zero local dependencies.
    Supports SVG, PNG, PDF output formats natively."""
    import urllib.request
    import urllib.error

    ext = out.suffix.lower()
    # Kroki supports svg, png, pdf for PlantUML
    kroki_fmt_map = {".svg": "svg", ".png": "png", ".pdf": "pdf"}
    kroki_fmt = kroki_fmt_map.get(ext, "svg")
    url = f"https://kroki.io/plantuml/{kroki_fmt}"
